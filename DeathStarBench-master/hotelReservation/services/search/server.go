...
import (
	...
	"github.com/delimitrou/DeathStarBench/tree/master/hotelReservation/overloadcontrol"
...
type Server struct {
	...
	OCType     overloadcontrol.OCType
	oc         *overloadcontrol.ServiceOC
}
...
func (s *Server) Run() error {
	...
	s.oc = overloadcontrol.NewServiceOC("search", s.OCType)

	interceptors := []grpc.UnaryServerInterceptor{
		otgrpc.OpenTracingServerInterceptor(s.Tracer),
	}
	if si := s.oc.ServerInterceptor(); si != nil {
...
func (s *Server) getGprcConn(name string) (*grpc.ClientConn, error) {
	dialOpts := []dialer.DialOption{dialer.WithTracer(s.Tracer)}
	if ci := s.oc.ClientInterceptor(); ci != nil {
		dialOpts = append(dialOpts, dialer.WithClientInterceptor(ci))
	}
	if s.KnativeDns != "" {
		return dialer.Dial(
