...
kubectl create configmap msgraph-config --from-file="${SCRIPT_DIR}/msgraph.yaml"
...
if [[ $METHOD == *"hotel"* ]]; then
  ...
  ./populate/populate -frontend="$frontend_address" -user="$user_address" -num_of_users=1000 -hotels_file="${SCRIPT_DIR}/experiments/hotel/data/hotels.json" > popu.output 2>&1
