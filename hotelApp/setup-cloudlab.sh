...
clone_repo() {
    ...
    if check_dir_exists ${repo}; then
        ...
        GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=accept-new" git clone https://github.com/Jiali-Xing/${repo}.git
    else
        echo "${repo} already exists."
    fi
}

# Clone the complete experiment repository (includes hotelApp, gateways, and hotelproto)
if check_dir_exists "Experiments-gateway-microservice-system"; then
    echo "Cloning experiment repository..."
    GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=accept-new" git clone https://github.com/GaussGeorge/Experiments-gateway-microservice-system.git
else
    echo "Experiments-gateway-microservice-system already exists."
fi

# Also clone hotelproto separately for proto files used by ghz
...
cd Experiments-gateway-microservice-system/hotelApp
