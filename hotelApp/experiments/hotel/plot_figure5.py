...
Usage:
    python3 plot_figure5.py [--results-dir results/figure5] [--slo 60]
"""
...
SLO_MS = 60
BIN_SIZE_S = 0.5  # 500ms time bins for rolling stats
...
def compute_time_series(records, slo_ms, bin_size_s, total_duration_s=15.0):
    """Compute binned time-series statistics.
    
    Returns dict with arrays (one value per time bin):
        time_centers, avg_latency_ms, p95_latency_ms,
        goodput_rps, slo_violation_rps, dropped_rps, rate_limited_rps
    """
    ...
    avg_lat = np.zeros(n_bins)
    p95_lat = np.zeros(n_bins)
    goodput = np.zeros(n_bins)
    slo_viol = np.zeros(n_bins)
    dropped = np.zeros(n_bins)
    rate_limited = np.zeros(n_bins)
    ...
    for i in range(n_bins):
        ...
        for r in bin_records:
            ...
            if error:
                ...
                if 'resource exhausted' in error_lower or 'unavailable' in error_lower:
                    n_dropped += 1
                elif 'rate' in error_lower or 'limit' in error_lower:
                    n_rate_limited += 1
                elif 'canceled' in error_lower or 'deadline' in error_lower:
                    n_dropped += 1
                else:
                    n_dropped += 1
...
def plot_figure5(all_series, slo_ms, output_path):
    """Generate Figure 5: 2 rows x 5 columns."""
    ...
    for col, method in enumerate(methods):
        ...
        mask_avg = ts['avg_latency_ms'] > 0
        mask_p95 = ts['p95_latency_ms'] > 0
        ...
        rate_lim_k = ts['rate_limited_rps'] / 1000.0
        ...
        ax_tp.fill_between(t, goodput_k + slo_viol_k,
                           ...
                           alpha=0.7, color='#E63946', label='Dropped')
        ax_tp.fill_between(t, goodput_k + slo_viol_k + dropped_k,
                           goodput_k + slo_viol_k + dropped_k + rate_lim_k,
                           alpha=0.7, color='#264653', label='Rate Limited')
...
def main():
    ...
    parser.add_argument('--results-dir', default=os.path.join(os.path.dirname(__file__), 'results', 'figure5'))
    ...
    for method in METHOD_ORDER:
        ...
        print(f"  {method}: {len(warmup_records)} warmup + {len(overload_records)} overload records")
